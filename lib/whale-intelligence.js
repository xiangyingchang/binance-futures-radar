...
const DIRECT_PROTOCOL_HINTS = Object.freeze({
  // Only used as hints. Proxy-based DeFi activity may not hit these addresses directly.
...
function units(rawValue, decimals) {
  ...
  return value / (10 ** Number(decimals || 0));
}

function makeFlow({ hash, timestamp, asset, amount, direction, kind, from, to, source, functionName }) {
  return {
    hash: String(hash || ''),
    timestamp: Number(timestamp || 0) * (Number(timestamp || 0) < 10_000_000_000 ? 1000 : 1),
    asset,
...
function groupByTransaction(flows, calls, ethPriceUsd) {
  ...
  return [...map.values()].map((item) => {
    ...
    if (ethNetUsd >= 1_000_000 && item.stableNetUsd <= -500_000) {
      ...
      classification = 'LEVERAGE_BUILDING';
      confidence = 'MEDIUM';
    ...
    return {
      ...
      ethNet: item.ethNet,
      ethNetUsd,
      stableNetUsd: item.stableNetUsd,
      grossUsd: item.grossUsd,
      assets: [...item.assets],
      counterparties: [...item.counterparties].slice(0, 4),
      protocolHints: item.calls.map((call) => call.protocol).filter(Boolean),
      functionNames: item.calls.map((call) => call.functionName).filter(Boolean),
      classification,
      confidence,
    };
...
function deriveState(summary24h, txGroups) {
  const recentCalls = txGroups.filter((item) => item.timestamp >= Date.now() - 24 * 60 * 60 * 1000);
  const hasBorrow = recentCalls.some((item) => item.classification === 'LEVERAGE_BUILDING');
  const hasRepay = recentCalls.some((item) => item.classification === 'DELEVERAGING');

  if (summary24h.ethNetUsd >= 10_000_000 && summary24h.stableNetUsd <= -5_000_000) {
    return { state: 'ACCUMULATING', score: 80, reason: '24H ETH/WETH 净流入显著，同时稳定币净流出。' };
  }
  if (summary24h.ethNetUsd <= -10_000_000 && summary24h.stableNetUsd >= 5_000_000) {
    return { state: 'DISTRIBUTING', score: -80, reason: '24H ETH/WETH 净流出显著，同时稳定币净流入。' };
  }
  ...
  return { state: 'NEUTRAL', score: 0, reason: '当前地址 24H 未出现足够明确的大额方向性链上流动。' };
}
...
function buildWhaleSnapshot({ entity, normalRows, internalRows, tokenRows, ethPriceUsd, now = Date.now() }) {
  ...
  const state = deriveState(summary24h, txGroups);

  const significantActions = txGroups
    .filter((item) => item.grossUsd >= 1_000_000 || Math.abs(item.ethNet) >= 500)
    .slice(0, 12);
  ...
  return {
    ...
    coverage: {
      ...
      limitation: '仅覆盖当前手工登记地址。代理钱包/新地址未登记时可能漏报；协议动作只有直接调用或可由资金流推断时才分类。',
    },
  };
}
