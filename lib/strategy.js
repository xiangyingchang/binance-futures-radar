...
function computeFundingArmState(fundingRows, currentFundingRate, now = Date.now(), lookbackHours = CONFIG.fundingArmedLookbackHours) {
  const rows = Array.isArray(fundingRows) ? fundingRows : [];
  const historyRates = rows
    .map((row) => finiteNumber(row?.fundingRate))
    .filter((value) => value !== null);
  const currentPercentile = percentileRank(historyRates, currentFundingRate);
  if (historyRates.length < 10) {
    return {
      fundingArmed: false,
      recentFundingPeakPercentile: currentPercentile,
      fundingArmedAt: null,
      fundingArmedUntil: null,
    };
  }
  ...
  const observations = rows
    .map((row) => ({
      timestamp: finiteNumber(row?.fundingTime),
      percentile: percentileRank(historyRates, row?.fundingRate),
    }))
    .filter((item) => item.timestamp !== null && item.timestamp >= cutoff && item.timestamp <= now && item.percentile !== null);

  if (currentPercentile !== null) observations.push({ timestamp: now, percentile: currentPercentile });

  let peak = null;
  let latestArmedAt = null;
  for (const item of observations) {
    if (peak === null || item.percentile > peak) peak = item.percentile;
    if (item.percentile >= CONFIG.fundingStrongPercentile) {
      if (latestArmedAt === null || item.timestamp > latestArmedAt) latestArmedAt = item.timestamp;
    }
  }

  return {
    fundingArmed: latestArmedAt !== null,
    recentFundingPeakPercentile: peak,
    fundingArmedAt: latestArmedAt,
    fundingArmedUntil: latestArmedAt === null ? null : latestArmedAt + (lookbackHours * 60 * 60 * 1000),
  };
}

function classifyPriceOiRegime(priceChangePct, oiChangePct) {
  const price = finiteNumber(priceChangePct);
  const oi = finiteNumber(oiChangePct);
  if (price === null || oi === null) return 'UNKNOWN';
  if (price === 0 || oi === 0) return 'FLAT_OR_MIXED';
  if (price > 0 && oi > 0) return 'PRICE_UP_OI_UP';
  if (price > 0 && oi < 0) return 'PRICE_UP_OI_DOWN';
  if (price < 0 && oi > 0) return 'PRICE_DOWN_OI_UP';
